...
- **`n` in the popup** runs `tmux command-prompt … new-window … 'zsh -ic claude'` (same template as `prefix C-c`, `-ic` so the `claude` alias expands) against the client behind the popup, then exits so the prompt is usable.
