...
- **Still tmux-only inside herdr:** `tmux-gf`/`gw` copy-mode openers (use `prefix e` = scrollback in nvim, then `gf`/`gx`), lazygit's `O` open-in-nvim (targets tmux window 1), `C-f` vimtest focus, `C-t` layout toggle, `C-j` join / `C-i` insert window, the git-flow status widget (flow runs in the foreground popup instead).
