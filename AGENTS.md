...
- **State comes from the registry `status`:** `busy|shell`→working (yellow ●), `waiting`→blocked (red ◉; set for permission dialogs, AskUserQuestion, sandbox/worker requests), `idle`→idle (dim ○, no fg so it inherits the theme). ANSI palette only so ghostty-mirror themes apply. The popup re-reads the registry every 500 ms (`Input::Tick`) and keeps the selection by pid.
- **Status line:** `status-right` runs `#(tmux-agents status)` (tmux markup, prefixed by Nerd Font md-creation U+F0674 because cod-claude is missing from FiraCode NF 3.4.0: `#[fg=red,bold]◉ n` blocked, yellow `● n` working, `#[dim]○ n` idle so it follows the theme's status-right-style, `none` when no sessions so the config-level `|` separator never dangles) at `status-interval 1`. Every other `#()` widget is wrapped in `scripts/tmux-cached <ttl> <cmd>` (5s TTL, cache in `~/.cache/tmux-cached/`) so the 1s interval does not rerun `git status`/`tsh`/`tailscale` each second. `~/.tmux_work.conf` overrides `status-right` and must carry the same segments.
...
- **Row layout:** `glyph word  label  <reason · title | ~/cwd>  session:window  age`. Row order is fixed while the popup is open (`merge_keeping_order`): new sessions append, gone ones drop, so rows never jump under the cursor; the initial order is blocked, working, idle. The middle column is ellipsis-truncated so the right column always fits. Title stripping removes any leading non-alphanumeric glyph + space (Claude uses `✳` and spinner glyphs).
- **Layout tests are `insta` snapshots** in `tests/snapshots.rs`; after an intentional layout change run `INSTA_UPDATE=always cargo test --test snapshots`, eyeball the `.snap` diff, and commit it. Behavior tests in `tests/picker.rs` assert only on row prefixes for the same reason.
- **`n` in the popup** runs `tmux split-window -h -c '#{pane_current_path}' 'zsh -ic claude'` against the pane behind the popup (`-ic` so the `claude` alias expands). `command-prompt` does not work from a popup: the prompt shows, but its command dies with the popup client.
