...
Also install the gh-dash theme refresh agent — it watches
`~/.config/ghostty/theme-current` and restarts gh-dash in tmux when the
Ghostty theme changes, so gh-dash always matches the terminal's light/dark
colors (theme overlay is generated by `scripts/gh-dash-theme` and merged in
by the `gh()` wrapper in `.zshrc`):

```bash
ln -sf $HOME/dotfiles/com.gh-dash.theme-refresh.plist $HOME/Library/LaunchAgents/com.gh-dash.theme-refresh.plist
